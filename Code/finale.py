...
####### TABLE
        ...
        vib=telemetry_daily.groupby('month')['vibration'].mean()

        plt.figure(figsize=(20, 10))
        plt.plot(vib.index, vib.values, color='midnightblue', marker='o')

        # plt.title('Monthly Variation of Vibration')
        # plt.xlabel('Month')
        # plt.ylabel('Voltage')
        # plt.xticks(vib.index, month_names)
        # plt.grid(True)
        # st.subheader("Monthly Variation of Vibration")
        # st.pyplot()

        # volt=telemetry_daily.groupby('month')['volt'].mean()

        # plt.figure(figsize=(20, 10))
        # plt.plot(volt.index, volt.values, color='midnightblue', marker='o')
        ...
        # plt.grid(True)
        # st.subheader("Monthly Variation of Voltage")
        # st.pyplot()

        # rotate=telemetry_daily.groupby('month')['rotate'].mean()
        # plt.figure(figsize=(20, 10))
        # plt.plot(rotate.index, rotate.values, color='midnightblue', marker='o')
        ...
        # plt.grid(True)
        # st.subheader("Monthly Variation of Rotation")
        # st.pyplot()

        
        # st.title("PdM Telemetry Data")
# ....................
        # Calculate the monthly sum for each attribute
        # st.divider()
        # st.title("new")
        monthly_sum_volt = telemetry_daily.groupby('month')['volt'].sum()
        monthly_sum_pressure = telemetry_daily.groupby('month')['pressure'].sum()
        monthly_sum_rotate = telemetry_daily.groupby('month')['rotate'].sum()
        monthly_sum_vibration = telemetry_daily.groupby('month')['vibration'].sum()

        # plt.figure(figsize=(20, 10))
        # plt.plot(monthly_sum_volt.index, monthly_sum_volt.values, color='midnightblue', marker='o', label='Voltage')
        # plt.plot(monthly_sum_pressure.index, monthly_sum_pressure.values, color='blue', marker='o', label='Pressure')
        # plt.plot(monthly_sum_rotate.index, monthly_sum_rotate.values, color='green', marker='o', label='Rotate')
        # plt.plot(monthly_sum_vibration.index, monthly_sum_vibration.values, color='purple', marker='o', label='Vibration')

        # plt.title('Monthly Variation of Attributes')
        # plt.xlabel('Month')
        # plt.ylabel('Value')
        # plt.xticks(monthly_sum_volt.index, month_names)
        # plt.grid(True)
        # plt.legend()
        # st.subheader("Monthly Variation of Telemetry Attributes")
        # st.pyplot()
        ...
        # # Calculate the sum of vibration per machine
        # avg_vib = telemetry_daily.groupby('machineID')['vibration'].sum()

        # # Display the line plot of sum of vibration per machine
        # fig, ax = plt.subplots(figsize=(20, 6))
        # avg_vib.plot(kind='line', color='pink', ax=ax)

        # ax.set_title('Sum of Vibration per Machine')
        # ax.set_xlabel('Machine ID')
        # ax.set_ylabel('Sum of Vibration')
        # ax.grid(True)
        ...
        # st.write("Correlation between age and pressure:", corr1)

        # corr2 = df1['age'].corr(df['vibration'])
        # st.write("Correlation between age and vibration:", corr2)


        # st.divider()
        telemetry_daily['datetime'] = telemetry_daily['datetime'].astype(str)
        # df['datetime'] = df['datetime'].astype(str)
        # st.write(df['datetime'])
        ...
        # st.write("resample to weekly")
        # weekly_data = telemetry_daily.resample('W').mean()
        # st.write(weekly_data)

        # Calculate correlation matrix for weekly data
        # correlation_matrix = weekly_data.corr()
        # st.write(correlation_matrix)

        # Resample data to monthly frequency and calculate the mean
        # st.write("resample to monthly")
        # monthly_data = df.resample('M').mean()
        # st.write(monthly_data)

        # monthly_data = df.resample('M').mean()
        # st.write("correlation matrix for monthly resampling")
        # correlation_matrix = monthly_data.corr()
        # st.write(correlation_matrix)

        # Resample data to yearly frequency and calculate the mean
...
def page2():
    ...
    def main():
        ...
        # machine = machfail[machfail['machineID'] == machine_id]
        # machine= machine.sort_values('days')
        
        # # time_diff = machfail_sorted.groupby('machineID')['days'].diff()
        # # failure_dates = machine_df[machine_df['failure_encoded'] == machine_id]['days']
        # time_diff = machine['failure_encoded'].diff()
        # st.write(time_diff)
        # sum_time_diff=time_diff.sum()
        # st.write(sum_time_diff)
        # avg_days_between_failures = sum_time_diff.mean()
        # st.write("Average number of days between two failures for Machine ID:", avg_days_between_failures)

        # machfail_sorted = machfail.sort_values('days')
        # time_diff = machfail_sorted.groupby('machineID')['days'].diff()
        # failure_dates = machine_df[machine_df['failure_encoded'] == machine_id]['days']
        # time_diff = failure_dates.diff()
        # st.write(time_diff)
        # sum_time_diff=time_diff.sum()
        # st.write(sum_time_diff)
        # avg_days_between_failures = sum_time_diff.mean()
        ...
        # machfail_sorted = machfail.sort_values('days')
        # time_diff = machfail_sorted.groupby('machineID')['days'].diff()
        # mean_days_between_failures = time_diff.mean()
        # mean_days_between_failures = int(round(mean_days_between_failures, 3))
        # st.write("Mean number of days between two failures:", mean_days_between_failures)



        # # Filter the machine ID and drop the 'model' and 'age' columns
        # machine_info = machfail[machfail['machineID'] == machine_id][['machineID', 'failure_encoded']]
        # machine_info = machine_info.drop(['model', 'age'])

        # # Display the machine ID and failure information
        ...
        # Display the unique values in the 'errorID' column and the updated dataframe

        # st.write(df['errorID'].unique())
        ...
        # st.subheader("Daily variation of Errors")
        # df['datetime'] = pd.to_datetime(df['datetime'])
        # df['day'] = df['datetime'].dt.day
        # daily_errors = df.groupby('day')['errorID_encoded'].size()
        # plt.figure(figsize=(10, 6))
